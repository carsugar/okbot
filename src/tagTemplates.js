...
const DETOUR_HEADER = '# ⬥ ─── DETOUR ───⬥';
const DETOUR_FOOTER = '# ⬥ ──────────────⬥';
const TRAVEL_INFO_HEADER = '# ⬥ ─── TRAVEL INFO ───⬥';
const TRAVEL_INFO_FOOTER = '# ⬥ ───────────────────⬥';

const TRAVEL_EMOJIS = { taxi: '🚕', bus: '🚌', walking: '🚶' };
const TRAVEL_DURATION_LABELS = { taxi: 'Duration of Ride', bus: 'Duration of Ride', walking: 'Duration of Walk' };
const TRAVEL_VERBS = { taxi: 'take a taxi', bus: 'use the bus', walking: 'walk' };
...
function travelVerb(option) {
  return TRAVEL_VERBS[option.toLowerCase()] || `use the ${option.toLowerCase()}`;
}

function splitIntoQuartiles(min, max) {
  if (min === max) return [[min, min], [min, min], [min, min], [min, min]];
  const span = max - min;
  const b1 = min + Math.floor(span / 4);
  const b2 = min + Math.floor(span / 2);
  const b3 = min + Math.floor((3 * span) / 4);
  return [[min, b1], [b1 + 1, b2], [b2 + 1, b3], [b3 + 1, max]];
}
...
function buildIndividualCommand(option, duration, cost, inits) {
  ...
  if (costVal === 0) {
    const fixed = nums[0] ?? '?';
    const past = TRAVEL_FREE_VERBS[option.toLowerCase()] || `traveled by ${option.toLowerCase()} for`;
    const body = `You have ${past} **${fixed} ${unit}**, and spent $0. Please use the command \`!${inits}-Arrived\`.`;
    return [`Carlbot tag — \`${trigger}\``, `!tag add ${trigger.replace('!', '').toLowerCase()} ${body}`];
  }

  const [minV, maxV] =
    nums.length >= 2 ? [nums[0], nums[1]] : nums.length === 1 ? [nums[0], nums[0]] : [0, 0];
  ...
  const mimuVerb = TRAVEL_MIMU_VERBS[option.toLowerCase()] || `traveled by ${option.toLowerCase()} for`;
  const reply =
    ...
    `{modifybal: -${costNum}}`;
  return [`Mimu autoresponder — \`${trigger}\``, `/autoresponder add trigger:${trigger} reply:${reply}`];
}
...
function roadblockModal() {
  return {
    ...
    data: {
      ...
      components: [
        row({ type: 4, custom_id: 'leg_number', label: 'Leg Number', placeholder: 'e.g. 10', style: 1, max_length: 10 }),
        row({ type: 4, custom_id: 'roadblock_title', label: 'Roadblock Title', placeholder: 'e.g. WHO IS TRYNA CRASH OUT?', style: 1, max_length: 200 }),
        row({ type: 4, custom_id: 'location_name', label: 'Location Name', placeholder: 'e.g. Cajon de Maipo', style: 1, max_length: 100 }),
        row({ type: 4, custom_id: 'location_image_url', label: 'Location Image URL', placeholder: 'https://...', style: 1, max_length: 500 }),
        row({ type: 4, custom_id: 'clue_text', label: 'Roadblock Clue Text', placeholder: 'Paste the full clue description here...', style: 2, max_length: 1000 }),
      ],
    },
  };
}
...
function legStartModal(leg) {
  return {
    ...
    data: {
      ...
      components: [
        row({ type: 4, custom_id: 'departure', label: 'Departure city, country | image URL', placeholder: 'e.g. Santiago, Chile | https://...', style: 1, max_length: 600 }),
        row({ type: 4, custom_id: 'destination', label: 'Destination city, country | site | airport', placeholder: 'e.g. Beijing, China | Temple of Heaven | PEK Airport', style: 1, max_length: 400 }),
        row({ type: 4, custom_id: 'money', label: 'Leg Money (no $ needed)', placeholder: 'e.g. 15', style: 1, max_length: 20, required: false }),
        row({ type: 4, custom_id: 'flight_time', label: 'Flight Duration (no "flight" needed)', placeholder: 'e.g. 3 hours', style: 1, max_length: 100 }),
      ],
    },
  };
}
...
function taskModal() {
  return {
    ...
    data: {
      ...
      components: [
        row({ type: 4, custom_id: 'tag_name', label: 'Tag Name', placeholder: 'e.g. dopamine-land', style: 1, max_length: 50 }),
        row({ type: 4, custom_id: 'body', label: 'Task Description', placeholder: 'Full task instructions...', style: 2, max_length: 1000 }),
      ],
    },
  };
}
...
function travelModal(transportCsv, hasCustom) {
  const components = [
    row({ type: 4, custom_id: 'tag_name', label: 'Tag Name', placeholder: 'e.g. cdm-arrived, dopamine-land', style: 1, max_length: 50 }),
    row({ type: 4, custom_id: 'destination', label: 'Destination', placeholder: 'e.g. Cajón Del Maipo (blank = skip default welcome text)', style: 1, max_length: 200, required: false }),
    row({ type: 4, custom_id: 'extra', label: 'Extra content (optional)', placeholder: 'Predecided travel note, task description, warnings...', style: 2, max_length: 1000, required: false }),
  ];
  if (hasCustom) {
    components.push(
      row({ type: 4, custom_id: 'custom_transport', label: 'Custom transport name', placeholder: 'e.g. tuk tuk, railcar', style: 1, max_length: 50, required: false })
    );
  }
  return {
    type: 9,
...
function detourModal(leg) {
  return {
    ...
    data: {
      ...
      components: [
        row({ type: 4, custom_id: 'location', label: 'Location Name', placeholder: 'e.g. San José | https://...', style: 1, max_length: 300 }),
        row({ type: 4, custom_id: 'option1_name', label: 'Option 1 Name', placeholder: 'e.g. Mangled', style: 1, max_length: 50 }),
        row({ type: 4, custom_id: 'option1_desc', label: 'Option 1 Description', placeholder: 'Describe option 1 task...', style: 2, max_length: 500 }),
        row({ type: 4, custom_id: 'option2_name', label: 'Option 2 Name', placeholder: 'e.g. Tangled', style: 1, max_length: 50 }),
        row({ type: 4, custom_id: 'option2_desc', label: 'Option 2 Description', placeholder: 'Describe option 2 task...', style: 2, max_length: 500 }),
      ],
    },
  };
}
...
function singleTravelModal(transport, inits) {
  ...
  return {
    ...
    data: {
      ...
      title: `${cap} Details`,
      components: [
        row({ type: 4, custom_id: 'duration', label: 'Duration', placeholder: 'e.g. 40 minutes to 50 minutes.', style: 1, max_length: 100 }),
        row({ type: 4, custom_id: 'cost', label: 'Cost (blank = free)', placeholder: 'e.g. $7', style: 1, max_length: 50, required: false }),
      ],
    },
  };
}
  ...
  return {
    ...
    data: {
      ...
      components: transports.slice(0, 5).map((t) => {
        ...
        return row({ type: 4, custom_id: `t_${t}`, label: cap, placeholder: 'e.g. 130 to 140 minutes | $20', style: 1, max_length: 150, required: false });
      }),
    },
  };
}
...
function transportButtonMessage(selectedCsv = '') {
  ...
  return {
    ...
    components: [
      ...
      {
        type: 1,
        components: [{ type: 2, custom_id: `travel_go:${selectedCsv}`, label: 'Fill out details →', style: 1 }],
      },
    ],
  };
}
...
export function handleComponent(interaction) {
  ...
  if (action === 'tg') {
    ...
    const newCsv = ['taxi', 'bus', 'walking', 'custom'].filter((v) => selected.has(v)).join(',');
  ...
  if (action === 'travel_go') {
    ...
    const selected = new Set(selectedCsv ? selectedCsv.split(',') : []);
    const transportStr = ['taxi', 'bus', 'walking'].filter((v) => selected.has(v)).join(',');
...
export function handleModalSubmit(interaction, env, ctx) {
  ...
  switch (modalType) {
    case 'modal_roadblock': {
      ...
      const locInitials = location.split(/\s+/).filter(Boolean).map((w) => w[0]).join('').toLowerCase();
    ...
    case 'modal_task': {
      ...
      const content = ROUTE_INFO_HEADER + '\n\n' + `*${body}*` + '\n' + ROUTE_INFO_FOOTER;
      return new JsonResponse({
        type: 4,
        data: { content: `**Route Info tag (\`!${tag}\`)**\n\`\`\`\n!tag add ${tag} ${content}\n\`\`\``, flags: 64 },
      });
    }
    ...
    case 'modal_legstart': {
      ...
      const [depDisplay, depImage = ''] = depRaw.split('|').map((s) => s.trim());
      const destParts = destRaw.split('|').map((s) => s.trim());
      const dest = destParts[0];
      const site = destParts[1] || '';
      const airport = destParts[2] || '';
      const destSlug = (dest.split(/\s+/)[0] || 'dest').replace(/[^a-z0-9]/gi, '').toLowerCase();
      const destCountry = dest.includes(',') ? dest.split(',').pop().trim() : dest;

      const money = moneyRaw.replace(/^\$/, '').trim();
      const moneyDisplay = money ? `$${money}` : '';

      const ftNorm = flightRaw.replace(/\bhours\b/gi, 'hour');
      const flightTime = /\bflight\b/i.test(ftNorm) ? ftNorm : `${ftNorm} flight`;
    ...
    case 'modal_travel': {
      ...
      const customTransport = hasCustom ? getField(interaction, 'custom_transport') : '';

      const transportParts = transportCsv ? transportCsv.split(',').filter(Boolean) : [];
      ...
      if (dest) {
        ...
        if (transportParts.length) {
          paragraphs.push(`*Make your way by ${humanJoin(transportParts)} to ${destDisplay} to receive your next clue.*`);
        } else {
          paragraphs.push(`*Make your way to ${destDisplay} to receive your next clue.*`);
        }
        const inits = getInitials(dest);
        if (transportParts.length) {
          paragraphs.push(`*For more information on how to reach this destination, use the command \`!${inits}-Travel\`. If you would like to pause, use the command \`!Pause\`.*`);
        } else {
          paragraphs.push(`*Once you arrive, use the command \`!${inits}-Arrived\`.*`);
        }
      }
      ...
      if (dest && transportParts.length) {
        ...
        const followup =
          transportParts.length === 1
            ? {
                content: '-# One transport option — enter time & cost to generate the command',
                flags: 64,
                components: [{ type: 1, components: [{ type: 2, custom_id: `single_travel_btn:${transportParts[0]}:${inits}`, label: 'Enter time & cost →', style: 2 }] }],
              }
            : {
                content: `-# Next: generate the travel tag \`!${inits.toLowerCase()}-travel\``,
                flags: 64,
                components: [{ type: 1, components: [{ type: 2, custom_id: `travel_tag_btn:${transportsCsv}:${inits}`, label: 'Generate travel tag →', style: 2 }] }],
              };
    ...
    case 'modal_single_travel': {
      ...
      const [label, command] = buildIndividualCommand(transport, duration, cost, inits);
      return new JsonResponse({
        type: 4,
        data: { content: `**${label}**\n\`\`\`\n${command}\n\`\`\``, flags: 64 },
      });
    ...
    case 'modal_travel_info': {
      ...
      for (const { option, duration, cost } of entries) {
        ...
        const durationLabel = TRAVEL_DURATION_LABELS[option.toLowerCase()] || 'Duration';
        const cmdSuffix = travelCmdSuffix(option);
        sections.push(
        ...
        cmdSentences.push(`If you would like to ${travelVerb(option)}, use the command \`!${destInitials}-${cmdSuffix}\`.`);
      }
      ...
      if (entries.length > 0) {
        ctx.waitUntil(
          (async () => {
            for (const { option, duration, cost } of entries) {
              const [label, command] = buildIndividualCommand(option, duration, cost, destInitials);
              await sendFollowup(env, interaction.token, `**${label}**\n\`\`\`\n${command}\n\`\`\``);
            }
          })()
        );
      }

      return new JsonResponse({ type: 4, data: { content: tagMsg, flags: 64 } });
    }
    ...
    case 'modal_detour': {
      ...
      const [locationName, locationImage = ''] = locRaw.split('|').map((s) => s.trim());
      const locSlug = slugify(locationName);
      const slug1 = slugify(name1);
      const slug2 = slugify(name2);

      const cmdArrived = `!${locSlug}-Arrived`;
      const cmd1 = `!L${leg}-${slug1}`;
      const cmd2 = `!L${leg}-${slug2}`;
      const cmdSwitch = `!L${leg}-Switch`;
      const tagArrived = `${locSlug.toLowerCase()}-arrived`;
      const tag1 = `l${leg}-${slug1.toLowerCase()}`;
      const tag2 = `l${leg}-${slug2.toLowerCase()}`;
      const tagSwitch = `l${leg}-switch`;

      const locationLine = locationImage
        ? `*Welcome to [${locationName}](${locationImage}).*`
        : `*Welcome to ${locationName}.*`;

      const overviewContent =
        DETOUR_HEADER + '\n\n' +
        locationLine + '\n\n' +
        `**It's a DETOUR!** A detour is a choice between two tasks. Choose which one you want to complete.\n\n` +
        `Use \`${cmd1}\` for **__${name1}__**\n` +
        `Use \`${cmd2}\` for **__${name2}__**\n` +
        DETOUR_FOOTER;

      const opt1Header = `# ⬥ ─── DETOUR: ${name1} ───⬥`;
      const opt1Footer = '# ⬥ ' + '─'.repeat(`DETOUR: ${name1}`.length + 8) + '⬥';
      const opt1Content =
        opt1Header + '\n\n' +
        `*${desc1}*\n\n` +
      ...
      const opt2Header = `# ⬥ ─── DETOUR: ${name2} ───⬥`;
      const opt2Footer = '# ⬥ ' + '─'.repeat(`DETOUR: ${name2}`.length + 8) + '⬥';
      const opt2Content =
        opt2Header + '\n\n' +
        `*${desc2}*\n\n` +
        `*If you would like to switch, use the command \`${cmdSwitch}\`.*\n` +
      ...
      return new JsonResponse({ type: 4, data: { content: msgs[0], flags: 64 } });
    }
